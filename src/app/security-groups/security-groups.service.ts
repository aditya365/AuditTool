...
export class SecurityGroupsService {
  ...
  getSecurityGroupsData(account, region, vpc, application, securityGroup): Observable<securityGroup[]> {
    return this.http.get<securityGroup[]>(`${environment.apiendpoint}securityGroups.json?account=${account}&region=${region}&vpc=${vpc}&application=${application}&securityGroup=${securityGroup}`);
  }

  getSecurityGroupDetails(groupId: string): Observable<any> {
    return this.http.get<any>(`${environment.apiendpoint}details.json?groupid=${groupId}`);
  }
